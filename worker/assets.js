 ...
 */
async function downloadFromGCS(gcsUri, destPath, projectId) {
    ...
    if (!gcsUri.startsWith('gs://') && !gcsUri.includes('/')) {
        ...
        if (!data) {
            ...
            if (!data) {
                ...
                const broadFound = broadSnap.docs.find((d) => {
                    const dData = d.data();
                    return dData.name === gcsUri || dData.filename === gcsUri || dData.assetId === gcsUri;
                });
...
async function generateImagenImage(prompt, destPath, projectId) {
    ...
    console.log(`🖼️  [Vertex AI] Imagen 3 Generation (Stable): "${prompt.slice(0, 80)}..."`);
    const body = {
        instances: [{ prompt }],
        parameters: {
            sampleCount: 1,
    ...
    if (projectId) {
        try {
            ...
            await storage.bucket(process.env.GCS_BUCKET_NAME).upload(destPath, {
                destination: gcsDest,
                metadata: { contentType: 'image/jpeg' }
            });
            const db = new Firestore(commonCredentials);
            const gcsUrl = `gs://${process.env.GCS_BUCKET_NAME}/${gcsDest}`;
...
async function generateVeoVideo(prompt, destPath, projectId, duration = 5) {
    ...
    const body = {
        ...
        parameters: {
            sampleCount: 1,
            durationSeconds: Math.min(Math.max(duration, 5), 10),
            aspectRatio: '16:9',
        },
    };
    const response = await fetch(endpoint, {
